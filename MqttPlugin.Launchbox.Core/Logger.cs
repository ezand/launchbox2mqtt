namespace MqttPlugin.Launchbox.Core
{
    public static class Logger
    {
        ...
        public static void LogDiagnostics()
        {
            ...
            var diagPath = Path.Combine(Path.GetDirectoryName(LogFilePath) ?? ".", "launchbox2mqtt-diagnostics.txt");
            var diagnostics = Diagnostics.GetDiagnostics();

            try
            {
                File.WriteAllText(diagPath, diagnostics);
            }
            catch
            {
                ...
                try
                {
                    ...
                    File.WriteAllText(tempPath, diagnostics);
                }
                catch
                {
